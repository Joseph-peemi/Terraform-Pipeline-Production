pipeline {
    ...
    stages {
      ...
      steps {
        script {
          sh "docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:0.34.0 image ${IMAGE_NAME}:latest --no-progress --security-checks vuln --exit-code 0 --severity HIGH,CRITICAL"
        }
      }
        }
